...
function nicknack_cups_shortcode() {
	...
	?><div class="hsContent all-cups">
		<div class="wrapper">
			...
			<span class="cup-volume">0,1 l &nbsp; <strong>0,2 l</strong> &nbsp; 0,3 l</span>
...
// Capability to see form entries
function gform_entries_capability() {

	$admin = get_role( 'administrator' );
	$admin->add_cap( 'gravityforms_view_entries' );
	$admin->add_cap( 'gravityforms_create_form' );
	$admin->add_cap( 'gravityforms_edit_forms' );

	$editor = get_role( 'editor' );
	$editor->remove_cap( 'gravityforms_view_entries' );

	// Nicknack user
	$user = new WP_User( 2 );
	$user->add_cap( 'gravityforms_view_entries' );
}
add_action( 'init', 'gform_entries_capability' );
	...
	if ( $result['is_valid'] /*&& ( $field->cssClass != 'cup-color-sample-wrapper' || ($field->cssClass == 'cup-color-sample-wrapper' && $value))*/ ) {
		...
		// Hotcup cap
			...
			if ( $variant=='potisk' ) {
				...
				// Selected Color field check
				if( strpos($field->cssClass,'cup-color-sample-wrapper')!==false && $value<250 ) {
